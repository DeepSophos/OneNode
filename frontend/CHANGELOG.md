...
- **🔗 Built-in LiteLLM Proxy**: Now includes LiteLLM proxy within ChatUI for enhanced functionality.

  - Easily integrate existing LiteLLM configurations using `-v /path/to/config.yaml:/app/backend/data/litellm/config.yaml` flag.
  - When utilizing Docker container to run ChatUI, ensure connections to localhost use `host.docker.internal`.
...
- Ollama is no longer required to run ChatUI.
- Access our comprehensive documentation at [ChatUI Documentation](https://docs.openwebui.com/).
