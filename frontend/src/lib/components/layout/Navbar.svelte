<script lang="ts">
	...
	import ScopeSelector from '../chat/ScopeSelector.svelte';
	...
	const shareChat = async () => {
		toast.success($i18n.t('This function has not implemented, please wait...'));
	};
...
<nav id="nav" class=" sticky py-3 border-gray-300 dark:border-gray-700 border-b  top-0 flex flex-row justify-center z-30">
	<div
	...
	>
		...
		<div class="flex items-center w-full max-w-full">
			<div class="w-full flex-1 overflow-hidden max-w-full  border-gray-300 ">
				<ScopeSelector bind:selectedScopes />
			</div>
			<div class="self-start flex flex-none items-center">
				...
				<!--	>-->
				<!--		<button-->
				<!--			class="hidden cursor-pointer p-1.5 flex dark:hover:bg-gray-700 rounded-full transition"-->
				<!--		>-->
				<!--			<div class=" m-auto self-center">-->
				<!--				<svg-->
				<!--					xmlns="http://www.w3.org/2000/svg"-->
				...
				<!--	</Menu>-->
				<!--{/if}-->
				<Tooltip content="{$i18n.t('Open new chat')}">
					<button
						id="new-chat-button"
						class=" cursor-pointer p-1.5 flex dark:hover:bg-gray-700 rounded-full transition"
						on:click={() => {
