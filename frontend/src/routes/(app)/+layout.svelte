<script lang="ts">
	...
	beforeNavigate(({ from, to, cancel }) => {
        if ((from?.route.id === '/(app)/c/[id]' && to?.route.id === '/(app)/rightpanel/[path]') ||
		  (from?.route.id === '/(app)' && to?.route.id === '/(app)/rightpanel/[path]')) {
			if (to?.params?.path) {
				rightpanel_params.set([to.params.path + to.url.search])
			}
			cancel();
        }
  });
...
{#if loaded}
	...
	<div class="app relative">
		{#if !['user','kbman', 'admin'].includes($user.role)}
					...
					<div class="m-auto pb-44 flex flex-col justify-center">
						<div class="max-w-md">
							<div class="text-center dark:text-white text-2xl font-medium z-50">
								账户激活待处理<br /> 请联系管理员获取Sapientia访问权限
							</div>

							<div class=" mt-4 text-center text-sm dark:text-gray-200 w-full">
								您的账户当前处于待激活状态。请联系管理员获取Sapientia访问权限。管理员可以在管理面板中管理用户状态。
							</div>
		...
		<div
			class=" text-gray-700 dark:text-gray-100 bg-gray-50 dark:bg-gray-900 min-h-screen overflow-auto flex flex-row"
		>
			<Sidebar />
